...
def create_grid_list(parent, items, item_type):
    ...
    for idx, item in enumerate(items):
        ...
        if item_type == 'clinic':
            ...
            edit_button = Button(item_frame, text="Edit", bg="green", fg="white", width=10, height=1,
                                 font=('Helvetica', 12), command=lambda did=item['doc_name']: edit_doctor(did))
            edit_button.pack(side="right", padx=5, pady=5)

            # Delete button for doctors
            delete_button = Button(item_frame, text="Delete", bg="red", fg="white", width=10, height=1,
                                   font=('Helvetica', 12), command=lambda did=item['doc_name']: delete_doctor(did))
            delete_button.pack(side="left", padx=5, pady=5)
            ...
            edit_button = Button(item_frame, text="Edit", bg="green", fg="white", width=10, height=1,
                                 font=('Helvetica', 12), command=lambda uid=item['name']: edit_user(uid))
            edit_button.pack(side="right", padx=5, pady=5)
...
def edit_doctor(doctor_id):
    def update_doctor():
        updated_doctor = {
            ...
            'is_available': availability_entry.get()  # Assuming availability is stored as boolean
            # Add more fields as needed
        }
    ...
    Label(edit_doctor_window, text="Availability   : ", font=('Helvetica', 12)).grid(row=3, column=0, padx=10, pady=5,
                                                                                     sticky="e")
    availability_entry = Entry(edit_doctor_window, width=25, font='Arial 19')
    availability_entry.grid(row=3, column=1, padx=10, pady=5, sticky="w")
    availability_entry.insert(0, doctor_data['is_available'])
...
def add_user():
    ...
    def submit_user():
        ...
        new_user = {
            ...
            'birthdate': f"{year_var.get()}-{month_var.get()}-{day_var.get()}",
        ...
        # Update user list display (replace with your update function)
        user_list()
    ...
    # User Name
    Label(add_user_window, text="User Name:", font=('Helvetica', 12)).grid(row=0, column=0, padx=10, pady=5, sticky="e")
    name_entry = Entry(add_user_window, width=30, font='Arial 12')
    name_entry.grid(row=0, column=1, padx=10, pady=5, sticky="w")

    # Gender
    Label(add_user_window, text="Gender:", font=('Helvetica', 12)).grid(row=1, column=0, padx=10, pady=5, sticky="e")
    gender_var = StringVar(add_user_window)
    gender_var.set("Male")  # Set default value
    gender_dropdown = OptionMenu(add_user_window, gender_var, "Male", "Female")
    gender_dropdown.config(width=20, font=('Arial', 12))
    gender_dropdown.grid(row=1, column=1, padx=10, pady=5, sticky="w")

    # Blood Type
    Label(add_user_window, text="Blood Type:", font=('Helvetica', 12)).grid(row=2, column=0, padx=10, pady=5,
                                                                            sticky="e")
    ...
    blood_type_dropdown.grid(row=2, column=1, padx=10, pady=5, sticky="w")

    # Birthdate
    Label(add_user_window, text="Birthdate:", font=('Helvetica', 12)).grid(row=3, column=0, padx=10, pady=5, sticky="e")

    # Year Dropdown
    Label(add_user_window, text="Year :", font=('Helvetica', 12)).grid(row=3, column=1, padx=10, pady=5, sticky="w")
    years = [str(year) for year in range(1950, date.today().year + 1)]
    year_var = StringVar(add_user_window)
    year_dropdown = ttk.Combobox(add_user_window, textvariable=year_var, values=years, state="readonly", width=10,
                                 font='Arial 12')
    year_dropdown.grid(row=3, column=2, padx=10, pady=5)

    # Month Dropdown
    Label(add_user_window, text="Month:", font=('Helvetica', 12)).grid(row=3, column=3, padx=10, pady=5, sticky="w")
    months = [str(month) for month in range(1, 13)]
    month_var = StringVar(add_user_window)
    month_dropdown = ttk.Combobox(add_user_window, textvariable=month_var, values=months, state="readonly", width=5,
                                  font='Arial 12')
    month_dropdown.grid(row=3, column=4, padx=10, pady=5)

    # Day Dropdown
    Label(add_user_window, text="Day:", font=('Helvetica', 12)).grid(row=3, column=5, padx=10, pady=5, sticky="w")
    day_var = StringVar(add_user_window)
    day_dropdown = ttk.Combobox(add_user_window, textvariable=day_var, state="readonly", width=5, font='Arial 12')
    day_dropdown.grid(row=3, column=6, padx=10, pady=5)
...
def edit_user(uid):
    ...
    # User Name
    Label(edit_user_window, text="User Name:", font=('Helvetica', 12)).grid(row=0, column=0, padx=10, pady=5,
                                                                            sticky="e")
    name_entry = Entry(edit_user_window, width=30, font='Arial 12')
    name_entry.grid(row=0, column=1, padx=10, pady=5, sticky="w")
    name_entry.insert(0, user_data['name'])

    # Gender
    Label(edit_user_window, text="Gender:", font=('Helvetica', 12)).grid(row=1, column=0, padx=10, pady=5, sticky="e")
    ...
    # Blood Type
    Label(edit_user_window, text="Blood Type:", font=('Helvetica', 12)).grid(row=2, column=0, padx=10, pady=5,
                                                                             sticky="e")
    blood_type_var = StringVar(edit_user_window)
    blood_type_var.set(user_data['bloodType'])  # Set default value
    blood_type_entry = Entry(edit_user_window, width=30, font='Arial 12', textvariable=blood_type_var)
    blood_type_entry.grid(row=2, column=1, padx=10, pady=5, sticky="w")

    # Submit Button
    submit_button = Button(edit_user_window, text="Update", command=submit_user_update, height=1, width=10,
                           font=('Arial', 20))
    submit_button.grid(row=4, columnspan=2, padx=10, pady=10, ipadx=50)
